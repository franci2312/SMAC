...
def generate_pcd_local_spots(V_original, T_nominal, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, percentage, color, seed):
    rng = np.random.RandomState(seed)
    V_i = V_original.copy()
    ...
    mask_spots = compute_mask_spots(V_i)
    T_i = pores_spacing(V_i, T_i, mask_spots, percentage, color, scaling_factor = 10.0, seed=seed)
...
def pores_spacing(v, t, mask, percentage, color=10.0, scaling_factor = 10.0, seed=123):
    ...
    new_texture = np.clip(new_texture, 0, scaling_factor)
    return new_texture
...
def process_pcd_gl_local_spots(args):
    ...
    V_i, T_i = generate_pcd_local_spots(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, percentage, color, seed)
    if method == 'gl':
...
def process_pcd_local_spots(args):
    V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seed, neig, sigma_eigsh, percentage, color, method = args
    V_i, T_i = generate_pcd_local_spots(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, percentage, color, seed)
    if method == 'lb':
...
def process_batch_pcds(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds, neig, sigma_eigsh, percentage, color, method, n_jobs):
    args_list = [(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seed, neig, sigma_eigsh, percentage, color, method) for seed in seeds]
    results = Parallel(n_jobs=n_jobs)(delayed(process_pcd_local_spots)(args) for args in args_list)
    evals_list, coeffs_list = zip(*results)
    return np.array(evals_list), np.array(coeffs_list)

def process_batch_pcds_gl(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds, neig, percentage, color, method, n_jobs):
    ...
    results = Parallel(n_jobs=n_jobs)(delayed(process_pcd_gl_local_spots)(args) for args in args_list)
    ...
    if method == 'lb':  
        for j in range(init, end):
            ...
            for k in range(init_col, end_col):
                ...
                evals_j, coeffs_j = process_batch_pcds(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds_j, neig, sigma_eigsh, percentage, col, method, n_jobs=args.n_jobs)
                end_time = time.time()
                np.save(os.path.join(output_folder_evals, f'evals_{k+1}.npy'), evals_j)
                np.save(os.path.join(output_folder_coeffs, f'coeff_{k+1}.npy'), coeffs_j)
        ...
        for j in range(init, end):
            ...
            for k in range(init_col, end_col):
                ...
                start_time = time.time()
                seeds_j = all_batch_seeds[j]
                evals_j = process_batch_pcds_gl(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds_j, neig, percentage, col, method, n_jobs=args.n_jobs)
                end_time = time.time()
                np.save(os.path.join(output_folder_evals, f'evals_{k+1}.npy'), evals_j)
                print(f"Saved evals for batch {k+1} to {output_folder}")
                elapsed = end_time - start_time
                print(f"Batch {k+1} completed in {elapsed:.2f}s ({elapsed/batch_size:.3f}s per PCD)")
                gc.collect()
        time.sleep(1)
    else:
        raise ValueError(f"Unknown method: {method}")
