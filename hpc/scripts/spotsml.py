...
def process_pcd_ml(args):
    V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, percentage, color, seed, neig, method = args
...
def process_batch_pcds_ml(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, percentage, color, seeds, neig, method, n_jobs):
    args_list = [(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, percentage, color, seed, neig, method) for seed in seeds]
    results = Parallel(n_jobs=n_jobs)(delayed(process_pcd_ml)(args) for args in args_list)
    return np.array(results)

def main():
    def parse_arguments():
        parser = argparse.ArgumentParser(description='Compute spots for ML methods.')
    ...
    for j in range(init, end):
        ...
        evals_j = process_batch_pcds_ml(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, perc, col, seeds_j, neig, method, n_jobs = args.n_jobs)
        end_time = time.time()
