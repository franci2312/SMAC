...
# process_pcd: generate and process a single point cloud
def process_pcd(args):
    V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seed, neig, sigma_eigsh, method = args
...
def process_batch_pcds(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seeds, neig, sigma_eigsh, method, n_jobs):
    args_list = [(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seed, neig, sigma_eigsh, method) for seed in seeds]
...
def process_batch_pcds_gl(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seeds, neig, sigma_eigsh, method, n_jobs):
    args_list = [(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seed, neig, sigma_eigsh, method) for seed in seeds]
    results = Parallel(n_jobs=n_jobs)(delayed(process_pcd_gl)(args) for args in args_list)
...
def main():
    ...
    idx = args.id
    main_seed = idx + 1  # use id as main seed
    ...
    print(f"Loaded nominal vertices with shape {V.shape}.")
    ...
    if method == 'lb':  
        for j in range(init, nbatches):
            ...
            evals_j, coeffs_j = process_batch_pcds(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seeds_j, neig, sigma_eigsh, method, n_jobs=args.n_jobs)
            end_time = time.time()
        ...
        for j in range(init, nbatches):
            ...
            start_time = time.time()
            seeds_j = all_batch_seeds[j]
            evals_j = process_batch_pcds_gl(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, seeds_j, neig, sigma_eigsh, method, n_jobs=args.n_jobs)
            end_time = time.time()
            np.save(os.path.join(output_folder_evals, f'evals_{j+1}.npy'), evals_j)
