...
# 3. generate_pcd: generate a point cloud with random subsampling and noise + local noise
...
def process_batch_pcds_ml(V_original, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds, neig, method, n_jobs):
...
def main():
    ...
    for j in range(init, end):
        ...
        evals_j = process_batch_pcds_ml(V, nominal_texture, min_size, max_size, sigma_shape, sigma_texture, sigma_oc_shape, seeds_j, neig, method, n_jobs = args.n_jobs)
        end_time = time.time()
