...
if __name__ == '__main__':
    ...
    df_color, results_color = run_oc(input_folder=input_folder, methods=methods, ranks=ranks, n3=n3, label=label, 
                                 stat_type=stat_type, k=k, B=B, maxrl=maxrl, nrep=30, cp=cp, levels=n_levels, seed=4321)

    df_color.to_csv(os.path.join(results_folder, f'df_{label}_{stat_type}_k{k}.csv'))
    df_color.index = [('SMAC' if m == 'LB' else m) for m in df_color.index]
    latex_str = df_color.to_latex(escape=False, column_format='l' + 'c'*n_levels)
    print("\nOC Results for color-only scenario:")
    print("=" * 40)
    print(latex_str)
    print("=" * 40)
    ...
    plot_arl_from_csv(
        ...
        ic_column=str(n3),  
        shifts=[0, 0.01, 0.05, 0.1, 0.15, 0.2, 0.25],
        title='', x_label='Shift',
        y_lim=(0, 120),
